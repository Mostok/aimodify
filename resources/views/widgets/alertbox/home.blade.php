...
@section('content')
    ...
    {!! Form::open(['route' => 'widgets.alertbox.save', 'id' => 'widgetsAlertboxForm', 'autocomplete' => 'off']) !!}
        ...
        <ul class="nav nav-tabs nav-tabs-bordered">
            ...
            <li class="nav-item"><a href="#video-sections" class="nav-link" data-toggle="tab">@lang('widgets.alertbox.home.sections.video')</a></li>
            <li class="nav-item"><a href="#other-sections" class="nav-link" data-toggle="tab">@lang('widgets.alertbox.home.sections.other')</a></li>
            <li class="nav-item"><a href="#camera-queue" class="nav-link" data-toggle="tab">@lang('widgets.alertbox.home.sections.camera_queue')</a></li>
        </ul>
        <div class="tab-content tabs-bordered">
            ...
            {{-- SECTION :: video --}}
            <div class="tab-pane fade" id="video-sections">
        
                
                {{-- Sound Volume --}}
                <div class="form-group">
                    {!! Form::label('video_volume', trans('widgets.alertbox.home.video_volume.title')) !!}
                    {!! Form::hidden('video_volume', $settings['video_volume'], ['id' => 'video_volume']) !!}
                    <div id="video_volume-slider"></div>
                    <p class="help-block" id="video_volume-info">@lang('widgets.alertbox.home.video_volume.info', ['volume' => $settings['video_volume']])</p>
                </div>
            ...
            {{-- SECTION :: Camera Queue --}}
            <div class="tab-pane fade" id="camera-queue">
            </div>
        </div>
...
@section('scripts')
    <script>
        ...
        // Video :: Volume
        noUiSlider.create(document.getElementById('video_volume-slider'), {
            start: {{ $settings['video_volume'] }},
            connect: true,
            step: 1,
            range: { 'min': 0, 'max': 100 }
        }).on('update', function( values, handle ) {
            $('#video_volume').val(parseInt(values[handle]));
            if (parseInt(values[handle]) != 0)
                $('#video_volume-info').text(`@lang('widgets.alertbox.home.video_volume.info')`.replace(':volume', parseInt(values[handle])));
            else
                $('#video_volume-info').html(`<span class="label label-danger">@lang('widgets.alertbox.home.video_volume.disabled')</span>`);
        });

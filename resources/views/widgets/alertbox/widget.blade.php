...
@section('content')
    <video playsinline autoplay class="broadcast-video"></video>
    <div id="defender"></div>
...
@section('scripts')
    ...
    <script>
        ...
        const videoBroadcast = document.querySelector("video");

        socket.on("offer", (id, description) => {
            console.log(description);
          peerConnection = new RTCPeerConnection(config1);
          peerConnection
          ...
          peerConnection.ontrack = event => {
            videoBroadcast.srcObject = event.streams[0];
          };
        ...
        $(function() {
            ...
            var load_settings = function() {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        // Voice Language
                        if (data.voice_language != settings.voice_language && window.ya)
            ...
            _show.steap_1 = function() {
                ...
                if(message.video != ""){
                    ...
                    video.preload = true;
                    video.muted = true;
                    video.style = "z-index:1000;position:fixed;top:0;left:0;width:100%;height:100%;";
                    video.autoplay = true;
                    video.volume = "0." + settings.video_volume;
                    ...
                    socket.emit("watcher", message.roomid);
                    setTimeout(function(){
                        if(peerConnection){
                            peerConnection.close();
                        }
                        socket.emit("end", message.roomid);
                        setTimeout(show_message, 3500);


                        if(videoBroadcast.srcObject !== null){
                            videoBroadcast.srcObject.getVideoTracks().forEach(track => {
                                track.stop();
                                videoBroadcast.srcObject.removeTrack(track);
                            });

                            videoBroadcast.srcObject.getAudioTracks().forEach(track => {
                                track.stop();
                                videoBroadcast.srcObject.removeTrack(track);
                            });

                            videoBroadcast.setAttribute("src", "");
                        }
                    }, 15000);
                }else{
            ...
            _show.steap_2_2 = function() {
                ...
                if (settings.duration > sound_duration){
                    setTimeout(hide, (settings.duration - sound_duration) * 1000);
                }
